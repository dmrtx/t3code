...
export const MuseSettings = makeProviderSettingsSchema(
  {
    // Off by default (like Cursor, Grok and OpenCode): the MSP binding is
    // new. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("muse").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Muse CLI binary (serves the MSP session host).",
