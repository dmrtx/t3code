/**
 * MuseDriver — `ProviderDriver` for the native Muse Code CLI over persistent MSP.
 *
 * Implements full session, turn, approval, user input, compaction, and event
 * streaming lifecycle via the Muse Session Protocol (JSON-RPC 2.0 over stdio).
 *
...
import { probeMuseSkills } from "./MuseSkills.ts";
...
export const MuseDriver: ProviderDriver<MuseSettings, MuseDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const discoverModels = yield* makeMuseModelDiscovery(effectiveConfig, processEnv);
      const checkProvider = checkMuseProviderStatus(
        effectiveConfig,
        processEnv,
        discoverModels,
      ).pipe(
      ...
      return {
        ...
        snapshotForCwd: (cwd) =>
          !effectiveConfig.enabled
            ...
            : Effect.all([
                snapshot.getSnapshot,
                probeMuseSkills(cwd, effectiveConfig, processEnv).pipe(
                  Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, spawner),
                  Effect.provideService(Path.Path, path),
                  Effect.mapError(
                    (cause) =>
                      new ProviderDriverError({
                        driver: DRIVER_KIND,
                        instanceId,
                        detail: `Failed to discover Muse skills for '${cwd}'`,
                        cause,
                      }),
                  ),
                ),
